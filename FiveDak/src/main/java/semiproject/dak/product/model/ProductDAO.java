...
public class ProductDAO implements InterProductDAO {
	...
	@Override
	public List<OrderDTO> productReview(String userid, int status) throws SQLException {
		...
		try {
			...
			String sql = " select P.product_name, B.brand_name, to_char(E.order_date , 'yyyy-mm-dd') as order_date , P.product_image_url, O.order_detail_product_id, (select count(order_quantity) from tbl_order_detail where review_status = 1), "
					   + " E.order_serial, P.product_id "
					   + " from tbl_product P JOIN tbl_order_detail O ON O.order_detail_product_id = P.product_id "
					   + " JOIN tbl_brand B on p.product_brand_id = B.brand_id JOIN tbl_order E on E.order_serial = O.fk_order_serial "
					   + " where order_member_id = ? and order_status = ? ";
			...
			while(rs.next()) {
				...
				oddto.setOrder_detail_product_id(rs.getInt(5));
				oddto.setOrder_quantity(rs.getInt(6));
				odto.setOrddt(oddto);
	...
	public int addReview(Map<String, String> paraMap)  throws SQLException {
		...
		try {
			...
			String sql = " insert into tbl_review(review_member_id, review_product_id, review_score, review_content, review_date)"
					   + " values( ?, ?, ?, ?, default) ";
			
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, paraMap.get("userid"));
			pstmt.setInt(2, Integer.parseInt(paraMap.get("product_id")) );
			pstmt.setDouble(3, Double.parseDouble(paraMap.get("rating")) );
