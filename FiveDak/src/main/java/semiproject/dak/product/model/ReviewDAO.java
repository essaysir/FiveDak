			...
			try {
				conn = ds.getConnection();
				 ...
				 String sql = " select ( select count(*) "
					 		...
					 		+ "         )as reviewCount, to_char(R.review_date, 'yyyy-MM-dd') as writeDate  "
				 			+ " from tbl_member M JOIN tbl_review R "
				 			+ " on M.member_id = R.review_member_id "
				 			+ " where M.member_id = ? ";
				 ...
				 if(rs.next()) { 
					 ...
					 map.put("reviewCount", rs.getString(1));
					 map.put("writeDate", rs.getString(2));
